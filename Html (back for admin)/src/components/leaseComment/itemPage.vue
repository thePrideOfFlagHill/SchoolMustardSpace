...
export default {
  ...
  methods: {
    ...
    allData () {
      ...
      this.$axios
        .get(this.$store.state.headPort + '/api/good/comment/query/all')
    ...
    goDel (id) {
      var that = this
      this.$axios
        .post(this.$store.state.headPort + '/api/good/comment/delete?id=' + id)
        .then(function (response) {
          that.$router.go(0)
        })
    }
  ...
  watch: {
    ...
    option (newData, oldData) {
      ...
      if (newData.contentType === '所属项目编号') {
        port = '/api/query/good/comment/table_id/'
      } else if (newData.contentType === '用户编号') {
        port = '/api/query/good/comment/user_id/'
      }
