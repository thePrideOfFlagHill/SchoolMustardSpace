...
export default {
  ...
  methods: {
    ...
    allData () {
      ...
      this.$axios
        .get(this.$store.state.headPort + '/api/lost_and_found/query/all')
  ...
  watch: {
    ...
    option (newData, oldData) {
      ...
      if (newData.contentType === '项目编号') {
        port = '/api/lost_and_found/query/id?id='
      } else if (newData.contentType === '用户编号') {
        port = '/api/lost_and_found/query/userId?userId='
      } else if (newData.contentType === '标题') {
        port = '/api/lost_and_found/query/like/title?text='
      } else if (newData.contentType === '内容') {
        port = '/api/lost_and_found/query/like/content?text='
      } else if (newData.contentType === '标签') {
        port = '/api/lost_and_found/query/like/label?text='
      }
      var page = this
      if (newData.content === '') {
        page.allData()
