...
public class ThreadPoolManager implements BeanFactoryAware {
    ...
    /**将任务加入token线程池*/
    public JsonResult addOrders(String token){
        System.out.println("此token号准备添加到线程池，token号：" + token);
        //验证当前进入的订单是否已经存在
        if (cacheMap.get(token) == null) {
            cacheMap.put(token, new Object());
            BusinessThread businessThread = new BusinessThread(token);
            threadPool.execute(businessThread);
        }
        return JsonResult.ok(taskService.getAllTask());
    }
    ...
    /**
     * 检查(调度线程池)，每秒执行一次，查看token的缓冲队列是否有token记录，则重新加入到线程池
     */
        ...
        public void run() {
            ...
            if(!msgQueue.isEmpty()){
                ...
                if (threadPool.getQueue().size() < WORK_QUEUE_SIZE) {
                    ...
                    threadPool.execute(businessThread);
                    System.out.println("(调度线程池)缓冲队列出现token业务，重新添加到线程池，token号："+orderId);
                }
            }
        }
    ...
    public void shutdown() {
        ...
        System.out.println("终止token线程池+调度线程池："+scheduledFuture.cancel(false));
        scheduler.shutdown();
        threadPool.shutdown();
